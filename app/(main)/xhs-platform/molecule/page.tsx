  ...
  useEffect(() => {
    const fetchCompareData = async () => {
      try {
        // 加载KOL数据
        setKolLoading(true);
        const kolRes = await axios.get('/api/feishu/XHSKOL');
  ...
  useEffect(() => {
    if (activeMainTab === 'hcpNonHcp' && activeSubTab === 'hcp') {
      const fetchHcpData = async () => {
        try {
          setHcpLoading(true);
          const res = await axios.get('/api/feishu/XHSHCP');
  ...
  useEffect(() => {
    if (activeMainTab === 'hcpNonHcp' && activeSubTab === 'nonHcp') {
      const fetchNonHcpData = async () => {
        try {
          setNonHcpLoading(true);
          const res = await axios.get('/api/feishu/XHSNONHCP');
  ...
  useEffect(() => {
    if (activeMainTab === 'kolUgc' && activeSubTab === 'kol') {
      const fetchKolData = async () => {
        try {
          setKolLoading(true);
          const res = await axios.get('/api/feishu/XHSKOL');
  ...
  useEffect(() => {
    if (activeMainTab === 'kolUgc' && activeSubTab === 'ugc') {
      const fetchUgcData = async () => {
        try {
          setUgcLoading(true);
          const res = await axios.get('/api/feishu/XHSUGC');
    ...
    if (activeMainTab === 'voicePlatformDistribution') {
      const fetchPlatformData = async () => {
        try {
          ...
          // 请根据实际接口地址修改
          const res = await axios.get('/api/feishu/XHSDistribution');
