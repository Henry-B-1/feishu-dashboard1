...
export default function SOVSOEPieChartPage() {
  ...
  const formatPieData = (indicatorType) => {
    ...
    const filteredData = rawData.filter(item => {
      ...
      return (
        fields?.['标题'] === '重点分子式声量及互动量表现（红书）' && // 精准匹配带红书的标题
        fields?.['拆分方式'] === '全量数据' &&
  ...
  const getEchartsPieOption = (indicatorType, pieData) => {
    ...
    return {
      title: {
        text: `重点分子式${indicatorType}值分布（红书·全量数据）`, // 标题增加红书标识
        left: 'center',
        top: 15,
        textStyle: {
          fontSize: 15,
  ...
  const getEchartsSingleBarOption = (title, brands, values, color) => {
    return {
      title: {
        text: `重点分子式${title.replace('品牌', '')}分布（红书·全量数据·${selectedMonth}）`, // 增加红书标识
        left: 'center',
        top: 15,
        textStyle: {
          fontSize: 15,
  ...
  const EmptyDataTip = ({ text }) => (
    <div style={{
      ...
      <span style={{
        fontSize: 14,
        fontWeight: 500
      }}>{text.replace('品牌', '分子式').replace('有效', '红书·有效')}</span>
    </div>
  );
  ...
  return (
    <div className={styles.chartContainer} style={{
      ...
      <div style={CHART_STYLE_CONFIG.parent}>
        ...
        >
          {loading ? (
            <LoadingSkeleton text="红书 SOV数据加载中..." />
          ) : sovPieData.length === 0 ? (
        ...
        >
          {loading ? (
            <LoadingSkeleton text="红书 SOE数据加载中..." />
          ) : soePieData.length === 0 ? (
      ...
      <div style={CHART_STYLE_CONFIG.parent}>
        ...
        >
          {loading ? (
            <LoadingSkeleton text="红书 总声量数据加载中..." />
        ...
        >
          {loading ? (
            <LoadingSkeleton text="红书 总互动量数据加载中..." />
          ) : barData.brands.length === 0 ? (
